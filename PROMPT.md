...
Model leans, prop leans and longshots are never favorites. X gets plays only, in one shape every time with the card: a **player prop**, a **team prop** (a side or a total) or the day's **fun parlay**, a researched favorite marked as such. The post is the play, its price, our number and one plain sentence of the reason; no link, no stat line, no recaps or scoreboards. A full game day carries them in that order of priority, and zero of any of them is fine.
