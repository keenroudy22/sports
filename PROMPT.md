...
The desk learns from its own graded record (DESK.md, Learning): it may make these rules stricter for a market that keeps losing to the closing line, and pause one that keeps losing at the strictest setting, but never looser than written here. Model leans, prop leans and longshots are never favorites. X gets plays only, in one shape every time with the card: a **player prop**, a **team prop** (a side or a total) or the day's **fun parlay**, a researched favorite marked as such. The post is the play, its price and units, our number and one plain sentence of the reason, tagged @Playbook; no link and no stat line. The morning after each game day a receipt lists every play that went out with its result, and Wednesday's covers the week. A game day with plays opens with a menu (games and times, never the side), and a day with nothing else gets the season's book in the evening; the model scoreboard stays on the site. A full game day carries them in that order of priority, and zero of any of them is fine.
