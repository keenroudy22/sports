...
5. **Research.** For each candidate, find a sourced reason the market might be wrong. Use:
   ...
   If you cannot name a sourced reason, pass, with one exception. A **model lean** is a total (never a spread, where the model carries no information against the close) whose calibrated chance from the desk clears its break-even by 1 point or more, priced on the board at its best book, with nothing sourced arguing against it. Publish it with `modelLean: true`, `favorite: false`, confidence 3 at 2 points or more and 2 below that, and a `why` that says plainly it rests on our number alone. At most four a day. Model leans give the model a live record of its own; they are never favorites and never go to X. A **prop lean** is the same idea on a player line: a priced player line on the board whose read says 60% or better on a settled role (three games this season, or eight or more for the same team last season) and clears its price by 5 points, since player chances are raw and uncalibrated. Publish it with `modelLean: true`, `favorite: false`, confidence 2, the best book's number and price, and a `why` that says the chance is uncalibrated. At most three per kickoff window (early, late, night), at most one per player, never a price worse than -200, and never a player from the day's longshot. Never on a player the injury report lists questionable or worse: the board marks them, their projected volume is already cut back, and their snaps are a coin flip before the number is. Check the number against the player's other markets first: a completions line above their attempts line, or any line the game cannot produce, is bad data, not an edge. Prop leans build the first graded history the calibration needs.
