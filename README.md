...
v2 does not beat the closing line and makes no claim to. Changes ship only when they beat the current parameters on the untouched holdout; `data/model/v2.1-tuning-*.json` records each attempt, including the ones that lost. Tested and not shipped: per-team continuity (`scripts/continuity.py`, 2026-09-23), which bends last season's weight by the share of a team's usage that has returned. The measure has spread (college teams return a third of last season's touches at the median, NFL teams two thirds) but it is unknown until a team has played, which is the stretch where last season matters most, and once known it moved the 2024 misses by 0.01 at most and lost on 2025. Also tested and not shipped: bending a player's target and carry shares by his recent snap share (`projections.SNAP_ROLE`, 2026-09-23), which raised every player miss at full strength and traded a small carries gain for a receiving-yards loss at half; the window's own half-life already follows a changing role. Also tested and not shipped: a quarterback-out term in the team number (`scripts/starters.py`, `qbOut` in the parameters, 2026-09-23), fitted from games the usual starter did not throw in (NFL about 1.8 points fewer scored, college 1.5). On the NFL, where the injury report gives a live flag, it left margins unchanged and worsened totals on 2025; on college it improved 2025 totals by 0.07 with the whole interval below zero, but the feed carries no college quarterback listings, so a shipped parameter would show a backtest gain the live forecast cannot deliver. It waits for a college availability source; until then the desk's gate refuses totals on games with a listed starter. Its 80% ranges held 78 to 82% of 2025 results. Early-season college mismatches are compressed toward the middle while teams have few games. A forecast counts from the moment the hosted workflow publishes it; nothing generated elsewhere is backfilled into the store.
